...
def _GenerateSubstructureDataset(
    ...
    substruct_counts = []
    ## Generate graph regression graphs
    for graph in graphs:
        assert graph.num_vertices() == num_vertices, (
            "num_vertices is %d" % graph.num_vertices()
        )
        _, counts = graph_tool.clustering.motifs(
            g=graph, k=substruct_graph.num_vertices(), motif_list=[substruct_graph]
        )
...
class SubstructureGeneratorWrapper(GeneratorConfigSampler):
    def __init__(
        self, param_sampler_specs, substruct, normalize_target=True, marginal=False
    ):
    ...
    def Generate(self, sample_id):
        ...
        node_features = [
            np.ones(shape=[graph.num_vertices(), 1], dtype=np.float32)
