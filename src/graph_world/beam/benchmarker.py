...
class BenchmarkGNNParDo(beam.DoFn):
    ...
    def __init__(
        ...
        self._benchmarker_classes = [
            benchmarker_wrapper().GetBenchmarkerClass()
            for benchmarker_wrapper in benchmarker_wrappers
        ]
        self._model_classes = [
            benchmarker_wrapper().GetModelClass()
            for benchmarker_wrapper in benchmarker_wrappers
        ]
        self._h_params = [
            benchmarker_wrapper().GetModelHparams()
            for benchmarker_wrapper in benchmarker_wrappers
        ]
        self._benchmark_params = [
            benchmarker_wrapper().GetBenchmarkParams()
            for benchmarker_wrapper in benchmarker_wrappers
        ]
        # /end alternate code.
    ...
    def process(self, element):
        ...
        for (benchmarker_class, benchmark_params, model_class, h_params) in zip(
            ...
            if num_tuning_rounds == 1 or self._tuning_metric == "":
                benchmark_params_sample, h_params_sample = SampleModelConfig(
                    benchmark_params, h_params
                )
                benchmarker = benchmarker_class(
                    element["generator_config"],
                    model_class,
                    benchmark_params_sample,
                    h_params_sample,
                    element["torch_data"],
                )
                benchmarker_out = benchmarker.Benchmark(
                    element,
                ...
                if num_tuning_rounds == 0:
                    ...
                    if benchmark_params is None:
                        ...
                        benchmark_params_product = list(
                            GetCartesianProduct(benchmark_params)
                        )
                ...
                for i in range(num_tuning_rounds):
                    if full_product:
                        if num_benchmark_configs > 0:
                            ...
                            benchmark_params_sample = benchmark_params_product[
                                benchmark_index
                            ]
                        else:
                ...
                val_scores = [
                    metrics[self._tuning_metric] for metrics in val_metrics_list
                ]
                test_scores = [
                    metrics[self._tuning_metric] for metrics in test_metrics_list
                ]
                ...
                output_data[
                    "%s__num_tuning_rounds" % benchmarker.GetModelName()
                ] = num_tuning_rounds
                if self._save_tuning_results:
                    output_data["%s__configs" % benchmarker.GetModelName()] = configs
                    output_data[
                        "%s__val_scores" % benchmarker.GetModelName()
                    ] = val_scores
                    output_data[
                        "%s__test_scores" % benchmarker.GetModelName()
                    ] = test_scores
