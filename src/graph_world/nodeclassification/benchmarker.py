...
class NNNodeBenchmarker(Benchmarker):
    def __init__(
        self, generator_config, model_class, benchmark_params, h_params, torch_data
    ):
        super().__init__(
            generator_config, model_class, benchmark_params, h_params, torch_data
        )
    ...
    def test(self, data, test_on_val=False):
        ...
        results = {
            ...
            "rocauc_ovr": sklearn.metrics.roc_auc_score(
                correct_onehot, pred_onehot, multi_class="ovr"
            ),
            "rocauc_ovo": sklearn.metrics.roc_auc_score(
                correct_onehot, pred_onehot, multi_class="ovo"
            ),
...
class NNNodeBaselineBenchmarker(Benchmarker):
    def __init__(
        self, generator_config, model_class, benchmark_params, h_params, torch_data=None
    ):
        super().__init__(
            generator_config, model_class, benchmark_params, h_params, torch_data=None
        )
        # remove meta entries from h_params
    ...
    def test(self, data, graph, masks, test_on_val=False):
        ...
        if test_on_val:
            ...
            for idx, node in enumerate(nodes_val):
                ...
                pprs = np.array(
                    gt.pagerank(
                        graph, damping=1 - self._alpha, pers=pers, max_iter=100
                    ).a
                )
    ...
    def Benchmark(
        ...
        try:
            ...
            out["val_metrics"].update(
                self.test(torch_data, gt_data, masks, test_on_val=True)
            )
            out["test_metrics"].update(
                self.test(torch_data, gt_data, masks, test_on_val=False)
            )
        except Exception:
