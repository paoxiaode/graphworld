...
def nodeclassification_data_to_torchgeo_data(
    nodeclassification_data: NodeClassificationDataset,
) -> Data:
    edge_tuples = []
    edge_feature_data = []
    for edge in nodeclassification_data.graph.iter_edges():
        ...
        edge_feature_data.append(nodeclassification_data.edge_features[ordered_tuple])
        edge_feature_data.append(nodeclassification_data.edge_features[ordered_tuple])

    node_features = torch.tensor(
        nodeclassification_data.node_features, dtype=torch.float
    )
    edge_index = torch.tensor(edge_tuples, dtype=torch.long)
    edge_attr = torch.tensor(edge_feature_data, dtype=torch.float)
    labels = torch.tensor(nodeclassification_data.graph_memberships, dtype=torch.long)
    return Data(
        x=node_features,
...
def sample_kclass_train_sets(
    example_indices: List[int], k_train: int, k_val: int
    ...
    # If the class has exactly 2 elements, assign one to train / test randomly.
        ...
        return ([example_indices[train_index]], [], [example_indices[test_index]])
    ...
    # element to test, and split the rest proportionally.
        ...
        num_test = 1
        num_train, num_val = get_num_train_val(
            len(example_indices) - 1, train_proportion
        )
    else:
        num_train = k_train
        num_val = k_val
...
def get_kclass_masks(
    ...
    for cluster_index, cluster in clusters.items():
        cluster_train_set, cluster_val_set, cluster_test_set = sample_kclass_train_sets(
            cluster, k_train, k_val
        )
    ...
    #         test_mask)
    return (
        torch.as_tensor(training_mask).reshape(-1),
