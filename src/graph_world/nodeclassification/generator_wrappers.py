...
class SbmGeneratorWrapper(GeneratorConfigSampler):
    ...
    def _AddLFRParameters(self):
        ...
        lfr_community_param_names = [
            "community_min_size_proportion",
            "community_max_size_proportion",
            "community_power_exponent",
        ]
        self._param_sampler_specs = {
            **{
                spec.name: spec
                for spec in self._lfr_params
        ...
        lfr_other_param_names = [
            "power_exponent",
            "avg_degree",
            "mixing_param",
            "max_degree_proportion",
        ]
        self._lfr_params = {
            spec.name: spec
            for spec in self._lfr_params
            if spec.name in lfr_other_param_names
        }
    ...
    def Generate(self, sample_id):
        ...
        if self._use_generated_lfr_communities:
            _, lfr_model = SimulateLFRWrapper(
                generator_config["nvertex"],
                self._lfr_params["avg_degree"].default_val,
                int(
                    self._lfr_params["max_degree_proportion"].default_val
                    * generator_config["nvertex"]
                ),
                self._lfr_params["power_exponent"].default_val,
                int(
                    generator_config["community_min_size_proportion"]
                    * generator_config["nvertex"]
                ),
                int(
                    generator_config["community_max_size_proportion"]
                    * generator_config["nvertex"]
                ),
...
class LFRGeneratorWrapper(GeneratorConfigSampler):
    def __init__(
        self, param_sampler_specs, marginal=False, normalize_features=True, num_tries=20
    ):
    ...
    def Generate(self, sample_id):
        ...
        lfr_data = GenerateLFRGraphWithFeatures(
            ...
            max_deg=int(
                generator_config["max_degree_proportion"] * generator_config["nvertex"]
            ),
