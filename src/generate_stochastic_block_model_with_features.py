...
from graph_world.nodeclassification.sbm_simulator import (
...
def main(argv):
    ...
    if sampler_config.generator_name == "StochasticBlockModel":
        ...
        data = GenerateStochasticBlockModelWithFeatures(
            ...
            prop_mat=generator_config.get(
                "prop_mat", np.ones((4, 4)) + 9.0 * np.diag([1, 1, 1, 1])
            ),
            feature_center_distance=generator_config.get(
                "feaeture_center_distance", 2.0
            ),
        ...
        raise ValueError(
            "Unknown Generator Name: {}".format(sampler_config.generator_name)
        )
